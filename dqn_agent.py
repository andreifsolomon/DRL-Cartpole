...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        if self.double_dqn:
            ...
            Q_targets_next = self.qnetwork_target(next_states.view(-1, 210, 160, 3)).detach().max(1)[0].unsqueeze(1)
            # Compute Q targets for current states
            Q_targets = rewards + (gamma * Q_targets_next * (1 - dones))

            # Get expected Q values from local model
            Q_expected = self.qnetwork_local(states.view(-1, 210, 160, 3)).gather(1, actions)
